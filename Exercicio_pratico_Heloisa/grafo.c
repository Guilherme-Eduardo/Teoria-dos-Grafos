...
/* Função que retorna o peso da aresta (Exercício 4)*/
// int peso_aresta (aresta e) {
//   return e->peso;
// }

char *atomo (vertice v) { return v->atomo; }
...
void adiciona_vertice(int id, char *atomo, grafo G) {
  ...
  novo_vertice->id = id; 
  strcpy(novo_vertice->atomo, atomo);
...
void imprime_grafo(grafo G) {
  printf("\nVertices: <id> <atomo> - [grau]( <fronteira> )");
  printf("\nVertices: ");
...
void imprime_vertice(vertice v) {
  ...
  printf("  %d %s - [%d]( ", vertice_id(v), atomo(v), grau(v));
...
/*Funcao que retorna um grafo H com o conjunto de vertices X contido em V(G)*/
grafo subgrafo_induzido (lista X, grafo G) {
  if (!G || !X) {
    fprintf (stdout, "Erro ao gerar grafo induzido de G[X]\n");
    return NULL;
  }
