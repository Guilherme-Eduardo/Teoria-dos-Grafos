...
cycles = list(nx.simple_cycles(G))  # Converte o gerador para lista

if cycles:
    largest_cycle = max(cycles, key=len)  # Encontrar o maior ciclo
    print(f"O maior ciclo simples possui {len(largest_cycle)} nós e é composto por: {largest_cycle}")
else:
    print("Não há ciclos simples no grafo.")
...
largest_com = max(nx.strongly_connected_components(G), key=len)
print("Maior componente fortemente conexa:", largest_com)

# Número de componentes fortemente conectados
num_com = nx.number_strongly_connected_components(G)
print("Número de componentes fortemente conectados:", num_com)
...
# Encontrar cliques maximais em digraph. Necessario converter para unidirecional
